...
uint8_t WT_Config_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG, 46+6+1+10+1+2+4);
...
uint8_t WT_Config_Save(void)
{
	...
	p = (uint8_t *) &WT_Config;
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG, 46+6+1+10+1+2+4);
	if(res == 0) 	return 0;
	else					return 1;
}
...
//	memset(WT_Config.SecKey,0,10);		//密钥
	...
	WT_Config.Server_IP[3] = 15;	 //服务器IP地址
	...
	WT_Config.Test_Wait = 0; //测试等待时间	
	WT_Config.Serial_Num = 0 ;//打印流水号
...
uint8_t WT_Config_Lockstat_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 47, 1);
...
uint8_t WT_Config_Lockstat_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 47, 1);
	if(res == 0) 	return 0;
	else					return 1;
}
...
uint8_t WT_Config_Testrule_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 48, 1);
...
uint8_t WT_Config_Testrule_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 48, 1);
	if(res == 0) 	return 0;
	else					return 1;
}
...
uint8_t WT_Config_Passwd_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 49, 6);
...
uint8_t WT_Config_Ipaddr_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 55, 4);
	if(res == 0) 	return 0;
	else					return 1;
}
...
uint8_t WT_Config_MAC_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 59, 6);
	if(res == 0) 	return 0;
	else					return 1;
}
...
uint8_t WT_Config_StudyMode_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 66, 1);
...
uint8_t WT_Config_StudyMode_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 66, 1);
	if(res == 0) 	return 0;
	else					return 1;
}
...
uint8_t WT_Config_Testwait_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 67, 1);
...
uint8_t WT_Config_Testwait_Save(void)
{
	...
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 67, 1);
	if(res == 0) 	return 0;
	else					return 1;
}

/**
  * @brief  WT_Config_Serial_Num_Read
  * @param  None.
  * @retval 0-ok, 1-error
  */
uint8_t WT_Config_Serial_Num_Read(void)
{
	...
	res = BSP_EEPROM_ReadBuffer(buf8, E2PROM_ADDR_WT_CONFIG + 68, 2);
	
	if(res == 0)
	{
		WT_Config.Serial_Num = (buf8[0]<<8) | (buf8[1]);
		WT_Config_Serial_Num_Save();
		return 0;
	}
	else
	{
...
uint8_t WT_Config_Serial_Num_Save(void)
{
	...
	p = (uint8_t *) &WT_Config.Serial_Num;
	res = BSP_EEPROM_WriteBuffer(p, E2PROM_ADDR_WT_CONFIG + 68, 2);
	if(res == 0) 	return 0;
	else					return 1;
}
